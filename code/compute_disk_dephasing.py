...
tau_values = [500, 1000, 2500, 3262, 3800, 7670]  # yr (3262 = canonical 1 kpc)
...
for alpha, label in models:
    ...
    dphi_3k, fP_3k, _, N_3k, eps_3k = compute_dephasing(3, 3262, alpha)
    ...
    print(f"    tau = 3262 yr:  eps(f_P) = {eps_3k:.1e}, "
          f"dPhi = {dphi_3k:.1f} rad ({dphi_3k/(2*np.pi):.1f} cycles)   [CANONICAL 1 kpc]")
...
print("f_dec = 3 nHz, tau = 3262 yr (canonical 1 kpc baseline)")
print("=" * 80)

for alpha, label in [(5.0/3, "WRONG (old paper)"), (2.0, "const da/dt"),
                      (7.0/3, "const torque"), (10.0/3, "const dE/dt")]:
    dphi, _, _, _, eps = compute_dephasing(3, 3262, alpha)
...
print("=" * 80)
for tau, label in [(1000, "1000 yr"), (3262, "3262 yr (canonical)"), (3800, "3800 yr")]:
