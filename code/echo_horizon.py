...
and plot contours of ρ_comb = 5, 10, 20.
...
The μAres Earth-term SNR far exceeds any reasonable detection threshold
across the entire plotted region under the proper integrated Fisher SNR
(Moore+2014 Eq. 19), so no μAres contour is drawn.
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
# ============================================================
# Compute ρ_comb over the grid (echo, PTA array)
# ============================================================
f_E = 1.0e-6  # Hz, fiducial Earth-term frequency
...
ax.clabel(cs, fmt=r'$\rho_{\rm comb}=%g$', fontsize=5.5, inline=True,
          inline_spacing=4, manual=[(6e8, 60), (2e8, 15), (3e9, 4e2)])
