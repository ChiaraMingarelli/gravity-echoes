...
IMPORTANT — this file does NOT cross-check against hardcoded "paper
values". The paper numbers in Section 4b were themselves produced by
running smbhb_evolution.py, so any hardcoded comparison would be
tautological (the library verifying itself). This script prints the
computed values; cross-validation against methods independent of
smbhb_evolution.py lives in the crosscheck_*.py files.
"""
...
def quick_taylorf2(m1, m2, chi1, chi2, kappa1, kappa2, f_E, D_L, t_span_yr):
    ...
    psi_4 = phi_4 + phi_4_SS  # TaylorF2 2pN (mass + SS)
    ...
    # TaylorF2 SPA perturbative cycle columns (Mingarelli 2012 convention)
    # psi_3_SO = (8 phi_3_SO - 5 tau_3_SO)/3 = 4 beta_so (Cutler-Flanagan 1994)
    psi_3_SO = (8 * phi_3_SO - 5 * tau_3_SO) / 3.0
    N_1pN = fd_pf * psi_2 * (v_P**(-3) - v_E**(-3))
    N_15pN = fd_pf * psi_3_mass * (v_P**(-2) - v_E**(-2))
    N_SO = fd_pf * psi_3_SO * (v_P**(-2) - v_E**(-2))
...
print("=" * 72)
r = quick_taylorf2(5e8, 5e8, 0.98, 0.98, 0, 0, 1e-6, 100, 3836)
print(f"Optimistic J1713 canonical baseline (L_p/c, theta=90): N_2pN = {r['2pN']:.1f}")

# ======================================================================
# Line 345: N_SO and radian phase for J1713 canonical baseline
# ======================================================================
print("\n" + "=" * 72)
print("Line 345: N_SO for optimistic J1713 canonical baseline (theta=90)")
print("=" * 72)
...
for M_tot, label in [(1e9, "Optimistic"), (5e8, "Typical")]:
    ...
    r = quick_taylorf2(M_tot / 2, M_tot / 2, 0.98, 0.98, 0, 0, 1e-6, 100, t_merge * 0.99)
    print(f"{label}: t_merge = {t_merge:.2f} yr, N_SO^(E) = {r['SO']:.1f}")

# ======================================================================
# Line 357: sigma_chi^(E) and rho_E from canonical first-principles calculation
# ======================================================================
print("\n" + "=" * 72)
print("Line 357: sigma_chi^(E) and rho_E  (inspiral-only Fisher, T_cap=1yr)")
print("=" * 72)
from compute_rho_E import sigma_chi_E_fisher, Mpc as MPC_M
chi = 0.98  # canonical aligned spin (migrated 2026-04-20 from 0.7)
for M_tot, label in [(1e9, "Optimistic"), (5e8, "Typical")]:
    sig, rho_LB, _, _, f_up, t_eff = sigma_chi_E_fisher(
        M_tot / 2, M_tot / 2, chi, 1e-6, 100.0 * MPC_M, T_cap_s=YR)
    print(f"{label}:  rho_E >= {rho_LB:.2e},  sigma_chi^(E) <= {sig:.2e}  "
          f"(t_eff={t_eff/YR:.2f} yr, f_up={f_up*1e6:.2f} uHz)")
...
print("=" * 72)
for M_tot, label in [(1e9, "Optimistic"), (5e8, "Typical")]:
    for tau, name in [(533, "J0437"), (3836, "J1713")]:
        r = quick_taylorf2(M_tot / 2, M_tot / 2, 0.98, 0.98, 0, 0, 1e-6, 100, tau)
...
print("=" * 72)
sc0437, r0437 = sigma_chi_pulsar(5e8, 5e8, 0.98, 1e-6, 100, 533, 0.11 / 156.96)
sc1713, r1713 = sigma_chi_pulsar(5e8, 5e8, 0.98, 1e-6, 100, 3836, 11.0 / 1176)
print(f"J0437: sigma_d/d = {0.11/156.96*100:.3f}%, N_SO = {r0437['SO']:.1f}, sigma_chi = {sc0437:.4f}")
print(f"J1713: sigma_d/d = {11./1176*100:.3f}%, tau = 3836 yr (theta=90 canonical), "
      f"N_SO = {r1713['SO']:.1f}, sigma_chi = {sc1713:.4f}")
...
print("=" * 72)
r_kpc = quick_taylorf2(5e8, 5e8, 0.98, 0.98, 0, 0, 1e-6, 100, tau_kpc)
req = 1e-3 * abs(r_kpc["SO"]) / (0.98 * r_kpc["f_P"] * tau_kpc * YR)
...
for N_psr, sig_dd, d_range, label in [
    ...
    for d_pc in np.linspace(d_range[0], d_range[1], N_psr):
        ...
        sc_i, _ = sigma_chi_pulsar(5e8, 5e8, 0.98, 1e-6, 100, tau_yr, sig_dd)
        inv_var += 1.0 / sc_i**2
...
for d_pc in np.linspace(500, 3000, 20):
    tau_yr = d_pc * PC / C_SI / YR
    sc_i, _ = sigma_chi_pulsar(5e8, 5e8, 0.98, 1e-6, 100, tau_yr, 0.001)
    inv_var += 1.0 / sc_i**2
...
phase_err = 2 * np.pi * N_SO_J1713 * sigma_chi_muares / 0.98
