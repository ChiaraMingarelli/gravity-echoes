...
class SMBHBEvolution:
    ...
    def _compute_pn_coefficients(self):
        ...
        # Aligned-spin simplification: SS contribution to the energy flux
        ...
        # SS contribution to the binding energy is dropped in the aligned-spin
        # limit: see the F4 comment above.  The 2pN SS phase correction is
        # carried exclusively by tau_4_SS / phi_4_SS in pn_decomposition().
        ...
        # Mass-only TaylorT4 2pN coefficient.  SS dropped for consistency
        # with F4 and A4 above — the aligned-spin SS enters only through
        # the TaylorF2 perturbative path (tau_4_SS / phi_4_SS).
        self.c2pn = (
    ...
    def pn_decomposition(self, t_span_yr: float, n_points: int = 10000) -> dict:
        ...
        tau_3_SO = (self.beta_so * 8.0) / 5.0                   # 1.5pN (SO)
        tau_4 = (3058673.0 / 508032 + 5429.0 * eta / 504
                 + 617.0 * eta ** 2 / 72)                       # 2pN (mass)
        tau_4_SS = -2.0 * self.sigma_ss                        # 2pN (SS)
        ...
        phi_3_SO = (5.0 / 2) * self.beta_so                     # 1.5pN (SO)
        phi_4 = (15293365.0 / 508032 + 27145.0 * eta / 504
                 + 3085.0 * eta ** 2 / 72)                      # 2pN (mass)
        phi_4_SS = -10.0 * self.sigma_ss                        # 2pN (SS)
        ...
        # At 2pN mass, psi_4_mass = phi_4_mass (standard TaylorF2 result;
        # the naive formula (8 phi_4 - 5 tau_4)/3 overcounts due to
        # cross-terms in the SPA integral that cancel at this order).
        # The same SPA coincidence holds for the 2pN SS contribution:
        # psi_4_SS = 5 tau_4_SS = phi_4_SS = -10 sigma_ss (matches LAL
        # TaylorF2). Therefore psi_4 (full) = phi_4 + phi_4_SS.
        # ==============================================================
        ...
        # SO contribution: TaylorF2 SPA perturbative (Mingarelli 2012 convention)
