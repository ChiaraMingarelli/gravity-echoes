...
def n_above(M_min_msun, bhmf=bhmf_lm24, log10M_hi=11.5):
    ...
    """
    def integrand(log10M):
        return bhmf(10**log10M) * np.log(10)  # bhmf returns dn/d(lnM); d(lnM) = ln(10)*d(log10M)
...
if __name__ == "__main__":
    ...
    # They predict 1-14 SMBHs with M_BH > 10^{10} in the MASSIVE survey volume
    # (2.05e6 Mpc^3, partial sky), NOT the full sphere at 108 Mpc.
    V_MASSIVE = 2.05e6  # Mpc^3
    N_check_full = N_above(1e10, 108)
    N_check_survey = n_above(1e10) * V_MASSIVE
    print(f"\n  Sanity check: N(>10^10)")
    print(f"    Full sphere (108 Mpc): {N_check_full:.1f}")
    print(f"    MASSIVE survey vol:    {N_check_survey:.1f}")
    print(f"    [LM24 predict 1-14 (90% CI) in MASSIVE survey vol]")
